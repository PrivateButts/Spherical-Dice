...
module dice() {
    ...
    difference() {
        union() {
            ...
            // Inner sphere
            sphere(r = (dice_diameter / 2) - (label_deboss / 2), $fn = dice_outer_resolution);
        }
        // Inner cavity
        union() {
            // Connecting pocket
    ...
    for (i = [0:pin_holes]) {
        rotate([ 0, 0, (360 / pin_holes * i) + pin_hole_rotation ])
            translate(v = [ dice_diameter / 2 - pin_hole_inset, 0, 0 ])
                cube([ pin_hole_diameter, pin_hole_diameter, pin_hole_depth * 2 ], center = true);
    }
    if (orientation_pin) {
        rotate([ 0, 0, orientation_pin_location ]) translate(v = [ dice_diameter / 2 - pin_hole_inset, 0, 0 ])
            cube([ pin_hole_diameter, pin_hole_diameter, pin_hole_depth * 2 ], center = true);
    }
}

module slicing_plane(angleX = 0, angleY = 0, angleZ = 0, size = 200) {
    rotate([ angleX, angleY, angleZ ]) translate([ 0, 0, -size / 2 ]) cube([ size, size, size ], center = true);
}
...
if (debug_cross_section) {
    ...
    translate([ dice_diameter + 5, 0, 0 ]) difference() {
        rotate([ -split_rot_x, 0, 0 ]) rotate([ 0, -split_rot_y, 0 ]) rotate([ 0, 0, -split_rot_z ]) difference() {
            dice();
    ...
    difference() {
        ...
        rotate([ 180 - split_rot_x, 0, 0 ]) rotate([ 0, -split_rot_y, 0 ]) rotate([ 0, 0, -split_rot_z ])
            intersection() {
            dice();
            slicing_plane(split_rot_x, split_rot_y, split_rot_z, 200);
        }
